import logging
from ml_toolkit.gridsearch.gridsearch1 import GridSearch
...
test_db = 'test_gridsearch1'

def python_options_2():
    eco_options = dict(
        dataset_setting=[
            {1: {'fea_type': {'all': {'slide_s': {60: {'slide_w':60}}}}}},
            {2: {'fea_type': {'raw':{'slide_s':{15: {'slide_w':15}}}}}},
            {3: {'fea_type': {'raw':{'slide_s':{30: {'slide_w':30}}}}}}
            ],
    ...
    svm = {
        'SVM': {
            'C': [1.0, 0.5],
            'kernel': ['linear', {'poly': {'degree': [2,3,4]}}, {'rbf': {'gamma': ['scale', 'auto']}}, 'sigmoid'],
        }
    }
    knn = {
        'kNN': {
    ...
    system_options = dict(expname='exp_eco_grid')
...
def insert(p):
    ...
    db.save_results(dataset, model, exp, result, perfs)
    # except Exception:
    #     logger.info(f'[exception] {i}! {p}')
    #     raise Exception(f'[exception] {i}! {p}')

def simulate_multiproces_insert():
    if os.path.isfile('./db/test_gridsearch1_mulp.db'):
        os.remove('./db/test_gridsearch1_mulp.db')
    options = python_options_2()
    total = 0
    ...
    with Pool(100) as pool:
        pool.map(insert, [p for p in enumerate(from_options(options))])
...
def query(p):
    i,p = p
    dataset,model,exp,result = from_param(p)
    # db = DatabaseManager(f'{test_db}_mulp.db','./db')
    db = DatabaseManager('exp_eco_grid.db', './db')
    perfs = db.get_perfs(dataset,model,exp,result)

def simulate_multiproces_select():
    options = python_options_2()
    total = 0
    from multiprocessing import Pool
    
    with Pool(100) as pool:
        pool.map(query, [p for p in enumerate(from_options(options))])

    logger.debug(f'[simulate] {total}')

# def test_multiprocess_query():
#     p = Profiler()
#     with p:
#         simulate_multiproces_select()
#     logger.info(f'\n{p.output_text()}')

def test_check_status():
    gs = GridSearch('exp_eco_grid', python_options_2())
    p = Profiler()
    with p:
        gs.check_status()
...
#     if os.path.isfile('./db/test_gridsearch1.db'):
#         os.remove('./db/test_gridsearch1.db')
#     options = python_options_2()
#     p = Profiler()
#     with p:
...
# def test_insert_grid():
#     gs = GridSearch('exp_eco_grid', python_options_2())
#     p = Profiler()
#     with p:
#         gs.create_grid_index()
#     logger.info(f'\n{p.output_text()}')